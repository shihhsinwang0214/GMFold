...
def my_fold(seq: str, temp: float = 37.0,) -> List[Struct]:
    """Fold the DNA sequence and return the lowest free energy score.
    ...
    APT.fit_fold( sequence=seq ,  n_tmpl=4, l_fix= 0 , filters = False)
    M = APT.M
    D = APT.dict_Sij

    v_cache, w_cache = my_cache(seq, temp,  M, D)
    n = len(seq)
...
def my_dg(seq: str, temp: float = 37.0) -> float:
    """Fold the sequence and return just the delta G of the structure
...
def my_cache(seq: str, temp: float = 37.0, M= None, D= None) -> Tuple[Structs, Structs]:
    ...
    # fill the cache
    my_w(seq, 0, n - 1, temp, v_cache, w_cache, emap, M, D)
...
def my_w(
    ...
    w1 = my_w(seq, i + 1, j, temp, v_cache, w_cache, emap, M, D)
    w2 = my_w(seq, i, j - 1, temp, v_cache, w_cache, emap, M, D)
    if M[i,j] != 1:
        w3 =  STRUCT_NULL
    else:
        w3 = my_v(seq, i, j, temp, v_cache, w_cache, emap, M, D)

    w4 = STRUCT_NULL
    for k in range(i + 1, j - 1):
       
        w4_test = my_multi_branch(seq, i, k, j, temp, v_cache, w_cache, emap, False, M, D)
...
def my_v(
    ...
    for bp in D[key]:
            ...
            # add V(i', j')
            e2_test += my_v(seq, i1, j1, temp, v_cache, w_cache, emap, M, D).e
    ...
    if not isolated_outer or not i or j == len(seq) - 1:
        for k in range(i + 1, j - 1):
            ...
            e3_test = my_multi_branch(seq, i, k, j, temp, v_cache, w_cache, emap, True, M, D)
...
def my_multi_branch(
    ...
    if helix:
        left = my_w(seq, i + 1, k, temp, v_cache, w_cache, emap,M, D)
        right = my_w(seq, k + 1, j - 1, temp, v_cache, w_cache, emap, M, D)
    else:
        left = my_w(seq, i, k, temp, v_cache, w_cache, emap, M, D)
        right = my_w(seq, k + 1, j, temp, v_cache, w_cache, emap, M, D)
    ...
    def add_branch(s: Struct):
        ...
        if len(s.ij) == 1:
            ...
            return
        for i1, j1 in s.ij:
            add_branch(my_w(seq, i1, j1, temp, v_cache, w_cache, emap, M, D))
    ...
    for index, (i2, j2) in enumerate(branches):
        ...
        if (i2, j2) != (i, j):  # add energy
            e_sum += my_w(seq, i2, j2, temp, v_cache, w_cache, emap, M, D).e
